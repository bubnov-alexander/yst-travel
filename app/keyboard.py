...
async def generate_orders_text_and_markup(orders_page, page, total_pages, is_sorted=False, is_month=False, month_number=0):
        ...
        for order in orders_page:
                ...
                orders_text += f"\n🟢**Дата приезда:** {str(order[1])}\n"
                orders_text += f"🔴**Дата выезда:** {str(order[2])}\n"
                orders_text += f"\n⏰️**Время приезда:** {str(order[3])}\n"
                orders_text += f"🗺**Маршрут:** {str(order[4])}\n"
                orders_text += f"\n📈**Количество катамаранов:** {str(order[5])}\n"
                orders_text += f"\n🤵**Имя:** {str(order[6])}\n"
                orders_text += f"📞 **Номер телефона:** {str(order[7])}\n"
                orders_text += f"\n💰**Цена заказа:** {str(order[8])}\n"
                orders_text += f"\n📝 **Дополнительные пожелания:** {str(order[9])}\n"
                orders_text += "━━━━━━━━━━━━━━━━━━━━\n"
...
async def info_text(date_id, date_start, date_end, time_start, route, quantity, customer_name, phone_number, price, additional_wishes):
        return (f'📝 Информация о заказе\n'
                ...
                f'📝 Дополнительные пожелания: {additional_wishes}\n'
                f'━━━━━━━━━━━━━━━━━━━━\n\n')